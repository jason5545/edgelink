...
final class LyraNetbusFrameTests: XCTestCase {
    ...
    func testLogiConnFrameGolden() {
        ...
        var expected = Data()
        LyraProtoWriter.appendVarintField(1, value: 0x10, to: &expected)
        LyraProtoWriter.appendVarintField(2, value: 0x20, to: &expected)
        LyraProtoWriter.appendVarintField(3, value: 0x01020304, to: &expected)
    ...
    func testExpressHandshakeEventFrameGolden() {
        let key = Data((0..<16).map { $0 })
        let frame = LyraExpressTLV.handshakeEventFrame(dataPort: 49902, key: key)
        var expected = Data()
        expected.append(contentsOf: [0x01, 0x01, 0xFF, 0xFF])
        expected.append(contentsOf: [0x00, 0x00, 0x00, 0x5B])
        expected.append(contentsOf: [0x00, 0x01])
        expected.append(contentsOf: [0x01, 0x00, 0x00, 0x01])
        expected.append(contentsOf: [0x00, 0x00, 0x00, 0x51])
        expected.append(contentsOf: [0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00])
        expected.append(contentsOf: [0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01])
        expected.append(contentsOf: [0x00, 0x03, 0x00, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00])
        expected.append(contentsOf: [0x00, 0x03, 0x00, 0x03, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0xC2, 0xEE])
        expected.append(contentsOf: [0x00, 0x05, 0x00, 0x04, 0x00, 0x00, 0x00, 0x10])
        expected.append(contentsOf: (0..<16).map { $0 })
        expected.append(contentsOf: [0x00, 0x01, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0x08])
