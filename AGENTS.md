...
- `[OBSERVED]` 2026-09-05（同日第三輪，outgoing dial 完整覆蓋與 mid-call channel loss 偵測）：jadx 確認 `handleRelayDialRequest` 在**每次** Mac 撥號時自動寫 `EXTRA_CALL_RELAYED=true`、`EXTRA_RELAY_ANSWERED=true`、`EXTRA_RELAY_DEVICE_ID=mac` 並 `setDeviceInRelay(mac)`——**outgoing dial 不需要 operate(0)**，pin 只靠來電接聽續期。`LyraRelayCallDialer` 只送 `relay://dial`（grep 確認），`noteIncomingCallAnswered` 唯一呼叫者是 `handleIncomingCallUIAnswer`——production 無路徑在撥號中誤送 operate。Mock 建模重點：`LyraRelayCallRole`（phone 的 relayCall client，phone-dials-Mac）與 `LyraRelayPhoneCallRole`（phone 的 relayPhoneCall server，Mac-dials-phone）是**獨立物件**，但 production TeleService 在所有 handler 間共享 `deviceInRelay`/`isCallRelayed`——測試必須手動同步（`phone.relayCall.deviceInRelay = "721572C3"` 等）。`LyraRelayCallRole` 的 `relayedNumbers`/`deviceInRelay` 改 `public var`（原 `private(set)`）供跨 role 同步；already-in-relay 分支補 `relayedNumbers.remove(address)`（TeleService `RelayMessageHandler.java:86` 的 releaseRelayExtra 語意）。`LyraRelayPhoneCallRole` 新增 `placeCallExtras`（dial 時寫的 Intent extras 快照）、`distAudioConnected`（`noteCallActive()` 時 deviceInRelay 非 nil 即翻 true，建模 `connectDistAudioDevice` 觸發鏈）。Production：`LyraRelayCallSession` 新增 `isCallActive` 狀態追蹤（callState 4=true、0/6/7/idle=false），`teardown()` 時若 `isCallActive` 為 true 則 log `relaycall.channel_lost_mid_call`（audio routing 會斷、協定層無復原機制——純診斷，不做自動處理）。E2E：`LyraPhoneServerIntegrationTests.testOutgoingDialFullChainNoOperateSent`（Mac dial→phone 記錄 extras+deviceInRelay→return channel→callState 4 ACTIVE→**斷言 operateRequests 為空**、`placeCallExtras` 正確、`distAudioConnected` true）；`testOperateDuringDialRejectedAndReleasesExtras`（建模 shared state 後 operate(0) → already-in-relay 500、relayedNumbers 清除、`answeredDeviceId` 維持 nil）。
